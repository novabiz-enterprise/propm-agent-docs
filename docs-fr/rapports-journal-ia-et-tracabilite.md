...
Autrement dit, une réponse d’agent ne devient pas automatiquement un document final : la chaîne reste **exécution → sortie structurée → artefact / Document PM → revue → décision aval**.
...
## Diff, lignée et gestion des versions
...
### Comment lire `Trace ID`, le coût, la confiance et la fraîcheur des sources
...
- **Cost** (`tokens` / `calls`) montre l’usage à l’exécution pour la transparence et l’enquête, pas un signal d’approbation métier à lui seul ;
- **Confiance** est un indice de relecture, pas une preuve autonome ;
- **Fraîcheur des sources** indique à quel point les preuves citées sont encore actuelles.
