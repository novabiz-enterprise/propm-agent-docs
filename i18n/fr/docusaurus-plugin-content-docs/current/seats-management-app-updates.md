...
Un utilisateur bloqué n’a pas forcément une question de connexion. Le blocage peut venir d’un manque de siège disponible, d’un utilisateur retiré à réassigner, ou d’un rôle, projet, binding, policy, configuration ou état health à corriger.
...
Elle ne relance pas l’offre Azure Marketplace, ne crée pas de nouveau groupe de ressources et ne recrée pas les ressources Azure déjà déployées.
...
Cette opération couvre le **rollout d’images applicatives**. Le bouton d’administration ne couvre pas les migrations de schéma base de données, la création de nouvelles ressources Azure, les changements d’architecture ou un nouveau déploiement Marketplace.
